...
public class MyDBHandler extends SQLiteOpenHelper {
    ...
    public void addToMeal(String dish) {
        ...
        if (c.getCount() <= 0) {
            ...
            @SuppressLint("Range") int amt = c.getInt(c.getColumnIndex("dishMeals"));
            if (amt == 0) {
                ContentValues values = new ContentValues();
    ...
    public int getGroceriesAmt(String ingredientName) {
        ...
        if (c.getCount() <= 0) {
            ...
            @SuppressLint("Range") int amt = c.getInt(c.getColumnIndex("groAmount"));
            c.close();
            return amt;
        }
    }
    ...
    public String getMeal(String date, String mealTime) {
        ...
        if (c.getCount() <= 0) {
            ...
            @SuppressLint("Range") String meal = c.getString(c.getColumnIndex("meals" + mealT));
            c.close();
            return meal;
        }
    }
    ...
    public String getName(int id) {
        ...
        Cursor c = db.rawQuery(Query, null);
        c.moveToFirst();
        @SuppressLint("Range") String name = c.getString(c.getColumnIndex("dishName"));
        db.close();
        return name;
    }
    ...
    public int getCalories(String dish) {
        ...
        Cursor c = db.rawQuery(Query, null);
        c.moveToFirst();
        @SuppressLint("Range") int amt = c.getInt(c.getColumnIndex("dishCalories"));
        db.close();
        c.close();
        return amt;
    }
